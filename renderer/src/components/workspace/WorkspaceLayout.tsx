    ...
    >
      ...
      <div className="relative z-10 flex flex-col w-full h-full">
        ...
        <div className="flex flex-row flex-1 overflow-hidden w-full relative z-10">
            ...
            <main className="flex-1 flex flex-col p-8 bg-transparent relative overflow-hidden pointer-events-auto">
                
                {/* GLOBAL WORKSPACE TOOLBAR (Restored to Node Area) */}
                <div className="w-full flex justify-center mb-4 z-40 relative">
                    <div className="flex flex-row items-center border border-[#2A2A35] rounded-full px-4 py-1.5 text-xs text-[#A0AEC0] bg-[#0B0B10] shadow-lg">
                ...
                {/* 3D GRAPH CANVAS INJECTION POINT (z-0, behind overlays) */}
                <div className="absolute inset-0 z-0">
                    <NeuralGraphDashboard />
                </div>

                {/* FILE SYSTEM SOLID OVERLAY */}
                {activeView === 'files' && (
                    <div className="absolute top-20 left-8 right-8 bottom-8 bg-[#0B0B10] z-50 p-8 border border-[#1E1E26] shadow-2xl overflow-y-auto rounded-lg">
                        <div className="flex justify-between items-center mb-2">
                            <h2 className="text-white font-bold tracking-widest text-lg whitespace-nowrap">[ FILE SYSTEM ]</h2>
                            <button onClick={() => setActiveView(null)} className="text-[#A0AEC0] hover:text-white font-bold text-xs">
                                CLOSE [x]
                            </button>
                        </div>
                ...
                {/* CHAT CONSOLE SOLID OVERLAY */}
                {activeView === 'chat' && (
                    <div className="absolute top-20 left-8 right-8 bottom-8 bg-[#0B0B10] z-50 p-8 border border-[#1E1E26] shadow-2xl overflow-y-auto rounded-lg flex flex-col gap-6">
                        <div className="mb-6 w-full">
                            <div className="flex flex-row items-center justify-between w-full mb-2">
                                <h2 className="text-white font-bold tracking-widest text-lg whitespace-nowrap">[ CHAT CONSOLE ]</h2>
                                <button onClick={() => setActiveView(null)} className="text-[#A0AEC0] hover:text-white font-bold text-xs whitespace-nowrap">
                                    CLOSE [x]
                                </button>
                            </div>
